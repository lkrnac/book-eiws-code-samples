...
public class ServerErrorITest extends AbstractTestNGSpringContextTests {
  @Autowired
  ...
  public void testGetUserDetails() throws IOException {
    ...
    RequestCreator requestCreator =
        RequestCreators.withPayload(new ClassPathResource(
            "testRequest-server-error.xml"));

    // WHEN
    ResponseActions response = wsClient.sendRequest(requestCreator);

    // THEN
    response.andExpect(ResponseMatchers.serverOrReceiverFault()).andExpect(
        ResponseMatchers.payload(new ClassPathResource(
            "testResponse-server-error.xml")));
  }
}
